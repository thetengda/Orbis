...
namespace gfgo
{
    ...
    bool PseudorangeRAWFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (!raw_factor_detail::linearizeCached(_message, _params, _bias_model, crd,
                                                ...
                                                0.0, 0.0, 0.0, false, false, false, _cache, linear))
        {
            return false;
        }
    ...
    bool MultiPseudorangeRAWFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (!raw_factor_detail::linearizeCached(_message, _params, _bias_model, crd,
                                                parameters[1][0], parameters[2][0], parameters[3][0],
                                                parameters[4][0], 0.0, 0.0, true, false, false, _cache, linear))
        {
            return false;
        }
    ...
    PseudorangeRAWIFBFactor::PseudorangeRAWIFBFactor(const RAWEquMsg &message, const t_gallpar &params, t_gprecisebiasFGO *bias_model)
        : _message(message), _params(params), _bias_model(bias_model)
    {
    }
    ...
    bool PseudorangeRAWIFBFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (!raw_factor_detail::linearizeCached(_message, _params, _bias_model, crd,
                                                parameters[1][0], parameters[2][0], parameters[3][0],
                                                0.0, parameters[4][0], 0.0,
                                                false, true, false, _cache, linear))
            return false;
        ...
        if (jacobians[0])
            for (int i = 0; i < 3; ++i)
                jacobians[0][i] = linear.sqrt_info * linear.coefficient[i];
        if (jacobians[1]) jacobians[1][0] = linear.sqrt_info * linear.coefficient[3];
        if (jacobians[2]) jacobians[2][0] = linear.sqrt_info * linear.coefficient[4];
        if (jacobians[3]) jacobians[3][0] = linear.sqrt_info * linear.coefficient[5];
        if (jacobians[4]) jacobians[4][0] = linear.sqrt_info * linear.ifb_coefficient;
        return true;
    }
    ...
    bool MultiPseudorangeRAWIFBFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (!raw_factor_detail::linearizeCached(_message, _params, _bias_model, crd,
                                                parameters[1][0], parameters[2][0], parameters[3][0],
                                                parameters[4][0], parameters[5][0], 0.0,
                                                true, true, false, _cache, linear))
            return false;
        ...
        if (jacobians[2]) jacobians[2][0] = linear.sqrt_info * linear.coefficient[4];
        if (jacobians[3]) jacobians[3][0] = linear.sqrt_info * linear.coefficient[5];
        if (jacobians[4]) jacobians[4][0] = linear.sqrt_info * linear.isb_coefficient;
        if (jacobians[5]) jacobians[5][0] = linear.sqrt_info * linear.ifb_coefficient;
        return true;
    }
}
