#include"multi_carrierphase_IF_integration_factor.h"

gfgo::MultiCarrierphaseIFINGFactor::MultiCarrierphaseIFINGFactor(const t_gtime &cur_time, const string &site, const t_gallpar  &params, const t_gsatdata &IF_sat_data, t_gprecisebiasFGO *bias_model, const pair<FREQ_SEQ, GOBSBAND> &freq_band1, const pair<FREQ_SEQ, GOBSBAND> &freq_band2, const Eigen::Vector3d &lever_arm) :
...
void gfgo::MultiCarrierphaseIFINGFactor::updatePara(t_gallpar & params_tmp, const Eigen::Vector3d & Pi, const double& Clk, const double&Trp, const double&ISB, const double&Amb) const
{
	params_tmp = _params;
...
bool gfgo::MultiCarrierphaseIFINGFactor::Evaluate(double const * const * parameters, double * residuals, double ** jacobians) const
{
	...
	if (jacobians)
	{
		...
		if (jacobians[3])
		{
			...
			jacobian_ISB = sqrt_info * B_IF.middleCols<1>(5);
			//cout << "ISB jacobian = " << jacobian_ISB << endl;
		}
		if (jacobians[4])
		{
			Eigen::Map<Eigen::Matrix<double, 1, 1, Eigen::RowMajor>> jacobian_AMB_IF(jacobians[4]);
