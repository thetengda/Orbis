#include "carrierphase_RAW_factor.h"
#include "raw_factor_common.h"

namespace gfgo
{
    CarrierphaseRAWFactor::CarrierphaseRAWFactor(const RAWEquMsg &message, const t_gallpar &params, t_gprecisebiasFGO *bias_model)
    ...
    bool CarrierphaseRAWFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (!raw_factor_detail::linearize(_message, _params, _bias_model, crd,
                                          ...
                                          0.0, parameters[4][0], false, true, linear))
        {
            return false;
        }
        ...
        if (jacobians[4])
            jacobians[4][0] = linear.sqrt_info * linear.amb_coefficient;
        return true;
    }
    ...
    bool MultiCarrierphaseRAWFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (!raw_factor_detail::linearize(_message, _params, _bias_model, crd,
                                          parameters[1][0], parameters[2][0], parameters[3][0],
                                          parameters[4][0], parameters[5][0], true, true, linear))
        {
            return false;
        }
        ...
        if (jacobians[5])
            jacobians[5][0] = linear.sqrt_info * linear.amb_coefficient;
        return true;
    }
}
