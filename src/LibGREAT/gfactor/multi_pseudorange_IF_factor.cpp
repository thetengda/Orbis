#include"multi_pseudorange_IF_factor.h"
#include"gproc/gfltmatrix.h"
gfgo::MultiPseudorangeIFFactor::MultiPseudorangeIFFactor(const t_gtime &cur_time, const string &site, const t_gallpar  &params, const t_gsatdata &IF_sat_data, t_gprecisebiasFGO *bias_model, const pair<FREQ_SEQ, GOBSBAND> &freq_band1, const pair<FREQ_SEQ, GOBSBAND> &freq_band2) :
...
void gfgo::MultiPseudorangeIFFactor::updatePara(t_gallpar & params_tmp, const Eigen::Vector3d & Pi, const double& Clk, const double&Trp, const double&ISB) const
{
	params_tmp = _params;
...
bool gfgo::MultiPseudorangeIFFactor::Evaluate(double const * const * parameters, double * residuals, double ** jacobians) const
{
	...
	//cout << "\n<< " << _IF_sat_data.sat() << " PseudorangeIFFactor " << endl;

	t_gallpar params_temp;
	updatePara(params_temp, Pi, Clk, Trp, ISB);
