#include"carrierphase_IF_integration_factor.h"

gfgo::CarrierphaseIFINGFactor::CarrierphaseIFINGFactor(const t_gtime &cur_time, const string &site, const t_gallpar  &params, const t_gsatdata &IF_sat_data, t_gprecisebiasFGO *bias_model, const pair<FREQ_SEQ, GOBSBAND> &freq_band1, const pair<FREQ_SEQ, GOBSBAND> &freq_band2, const Eigen::Vector3d &lever_arm) :
...
void gfgo::CarrierphaseIFINGFactor::updatePara(t_gallpar & params_tmp, const Eigen::Vector3d & Pi, const double& Clk, const double&Trp, const double&Amb) const
{
	params_tmp = _params;
...
bool gfgo::CarrierphaseIFINGFactor::Evaluate(double const * const * parameters, double * residuals, double ** jacobians) const
{
	...
	updatePara(params_temp, Pi_ARP, Clk, Trp, Amb);
