 ...
 * @brief        Main Function for GNSS RTK/PPP, Kalman-filter (FLT) or
 *               factor-graph optimization (FGO) selected by <gen>/<est>.
 * @version      1.0
 * @date         2025-11-04
 *
...
int main(int argc, char** argv)
{
	...
	// Which estimation engine runs: "FGO" (factor graph) or "FLT" (Kalman filter).
	const string est = gset.estimator();
	const bool isFGO = (est == "FGO");
	cout << "GREAT_PVT: estimation engine = " << gset.estimator() << endl;

	if (isFGO)
	{
		// Validate the FGO-only settings before any processing.
		try
		{
		...
		{
			cerr << "GREAT_PVT configuration error: " << error.what() << endl;
			return 1;
		}
		...
		if (configured_obs_combination == OBSCOMBIN::RAW_ALL)
			cout << "GREAT_PVT: RAW_ALL selects the uncombined SION/RAW PPP graph.\n";
		else if (configured_obs_combination == OBSCOMBIN::RAW_MIX)
			cout << "GREAT_PVT: RAW_MIX is currently unsupported by the FGO PPP graph; use IONO_FREE or RAW_ALL.\n";
	}
	...
	// vgpvt for the process of ppp: derived t_gpvtfgo (FGO graph) or base t_gpvtflt (filter).
	...
	for (size_t i = 0; i < inp.size() && itINP != inp.end(); ++i, ++itINP)
	{
		...
		if (tgcoder)
		{
			...
			// Put the gcoder into the gio (gio contains the gcoder, gcoder the gdata)
			tgio->coder(tgcoder);
	...
	// set antennas for satellites (must be before PCV assigning)
	...
	// the legacy filter adds UPD for any fix mode; the factor-graph engine only
	// needs the UPD products in SEARCH mode.
	bool want_upd_data;
	if (isFGO)
		want_upd_data = dynamic_cast<t_gsetamb*>(&gset)->fix_mode() == FIX_MODE::SEARCH;
	else
		want_upd_data = dynamic_cast<t_gsetamb*>(&gset)->fix_mode() != FIX_MODE::NO;
	if (gupd && want_upd_data && !isBase)
	{
